...
def curve_length(x, y):
    ...
    for i in range(len(x)-1):
        sum = sum + ((x[i]-x[i+1])**2 + (y[i]-y[i+1])**2)**0.5
    return sum

Mdb()

# execfile("Parameters_for_NonlinearBucklingCTLTs_Y.py")
...
w = 1.0 # Web
L = 3.0 # lumbus
Curve_L = (h_total - 2.0 * w - L)/4.0

x = np.linspace(w,10,10000)
pow_a = 0
a = 10**(pow_a)
y = a*(x-w)**2
...
for i in range(len(x)):
    if curve_length(x[0:i], y[0:i]) > Curve_L:
        max_index = i
        break
print(Curve_L)
print(max_index)
print(curve_length(x[0:max_index], y[0:max_index]))
...
N = 100
px = np.linspace(x[0], x[max_index], N)
py = a * (px - w)**2
...
mdb.models[LBModel].sketches['__profile__'].Spline(points=list)

mdb.models['LinearBucklingModel'].sketches['__profile__'].copyRotate(angle=
    180.0, centerPoint=(px[N-1], py[N-1]), objectList=(
    mdb.models['LinearBucklingModel'].sketches['__profile__'].geometry.findAt((
    px[N/2], py[N/2]), ), ))

mdb.models[LBModel].sketches['__profile__'].Line(
    point1=(w + (px[N-1] - w)*2.0, py[N-1]*2.0), 
    point2=(w + (px[N-1] - w)*2.0 + L/2.0, py[N-1]*2.0))

mdb.models[LBModel].sketches['__profile__'].ConstructionLine(
  point1=(w + (px[N-1] - w)*2.0 + L/2.0, py[N-1]*2.0), point2=(w + (px[N-1] - w)*2.0 + L/2.0, py[N-1]))

mdb.models[LBModel].sketches['__profile__'].copyMirror(
    mirrorLine=mdb.models[LBModel].sketches['__profile__'].geometry.findAt((w + (px[N-1] - w)*2.0 + L/2.0, py[N-1]), ), 
    objectList=(
    mdb.models[LBModel].sketches['__profile__'].geometry.findAt((w/2.0, 0.0), ), 
    mdb.models[LBModel].sketches['__profile__'].geometry.findAt((px[N/2], py[N/2]), ),
    mdb.models[LBModel].sketches['__profile__'].geometry.findAt((2.0*px[N-1] - px[N/2], 2.0*py[N-1] - py[N/2]), ),
    mdb.models[LBModel].sketches['__profile__'].geometry.findAt((w + (px[N-1] - w)*2.0 + L/2.0/2.0, py[N-1]*2.0), )
    ))
...
mdb.models[LBModel].parts['Part-1'].Set(faces=
    mdb.models[LBModel].parts['Part-1'].faces.findAt(((w/2.0, 0.0, 0.0), ), 
                                                       (((w + (px[N-1]-px[0])*2.0 + L/2.0)*2.0 - w/2.0, 0.0, 0.0), ), 
                                                       ), name='Set-Web')
mdb.models[LBModel].parts['Part-1'].Set(faces=
    mdb.models[LBModel].parts['Part-1'].faces.getByBoundingBox(w/2.0, -2*py[N-1], 0.0, (w + (px[N-1]-px[0])*2.0 + L/2.0)*2.0 - w/2.0, 2*py[N-1], l), name='Set-Flanges')
...
# # # #
VirtualNodes(mdb, LBModel, 'Ref-1', (w + (px[N-1]-w)*2.0 + L/2.0), 0.0, 0.0)
VirtualNodes(mdb, LBModel, 'Ref-2', (w + (px[N-1]-w)*2.0 + L/2.0), 0.0, l)
...
ParName = '-' + str(floor(abs(pow_a))).split('.')[0] + '_' + \
              str(float("%.2f" % floor(abs(pow_a)*10-10*floor(abs(pow_a))))).split('.')[0] + \
              str(float("%.2f" % abs(abs(pow_a)*100-100*floor(abs(pow_a)) - 10*floor(abs(pow_a)*10-10*floor(abs(pow_a)))))).split('.')[0] + '+' +\
...
jobName_Linear = 'CTLTs_LinearBuckling_Y-' + ParName
...
jobName_NonLinear = 'CTLTs_NonLinearBuckling_Y-' + ParName
...
i_num = 0
jobName_Restart = 'CTLTs_NonLinearBuckling_Y-Restart-' + ParName
while True:
...
Data = np.loadtxt(open(jobName_Restart + ".csv", "rb"), delimiter=",",
                      skiprows=1)
outfile = open('CTLTs_NonLinearBuckling_Y-' + ParName + '-FinalResults.csv', 'w')
...
jobName_PostBuckling = 'CTLTs_NonLinearPostBuckling_Y-' + ParName
...
i_num = 0
jobName_Restart_P = 'CTLTs_NonLinearPostBuckling_Y-Restart-' + ParName
while True:
...
Data = np.loadtxt(open(jobName_Restart_P + ".csv", "rb"), delimiter=",",
                      skiprows=1)
outfile = open('CTLTs_NonLinearPostBuckling_Y-' + ParName + '-FinalResults.csv', 'w')
outfile.write('Stiffness [N m/rad]' + ',' + 'NonlinearCriticalLoad [N m]' + '\n')
outfile.write(str(K[0]) + ',' + str(Data[0,1]) + '\n')
outfile.close()
