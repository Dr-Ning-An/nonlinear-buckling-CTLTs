...
# execfile('Parameters_for_TRAC_NonlinearBuckling_X.py')
...
jobName_Linear = 'TRAC_LinearBuckling_X-' + ParName
...
jobName_NonLinear = 'TRAC_NonLinearBuckling_X-' + ParName
...
i_num = 0
jobName_Restart = 'TRAC_NonLinearBuckling_X-Restart-' + ParName
while True:
...
Data = np.loadtxt(open(jobName_Restart + ".csv", "rb"), delimiter=",",
                      skiprows=1)
outfile = open('TRAC_NonLinearBuckling_X-' + ParName + '-FinalResults.csv', 'w')
...
jobName_PostBuckling = 'TRAC_NonLinearPostBuckling_X-' + ParName
...
i_num = 0
jobName_Restart_P = 'TRAC_NonLinearPostBuckling_X-Restart-' + ParName
while True:
...
Data = np.loadtxt(open(jobName_Restart_P + ".csv", "rb"), delimiter=",",
                      skiprows=1)
outfile = open('TRAC_NonLinearPostBuckling_X-' + ParName + '-FinalResults.csv', 'w')
outfile.write('Stiffness [N m/rad]' + ',' + 'NonlinearCriticalLoad [N m]' + '\n')
outfile.write(str(K[0]) + ',' + str(Data[0,1]) + '\n')
outfile.close()
